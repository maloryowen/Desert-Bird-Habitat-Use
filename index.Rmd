...
###Exploration
```{r, EDA, include=FALSE}
season.ssp <- site[,c(4, 12)] #pull season and species
season.ssp <- as.data.frame(table(season.ssp)) #freq of species in each season

season.tro <- site[,c(4, 14)] #pull season and trophic guild
season.tro <- as.data.frame(table(season.tro))

season.mig <- site[,c(4, 13)] #pull season and migratory class
...
#for both seasons
#mesos and species
meso.ssp <- site[,c(4, 19, 12)] #pull season, mesohabitat, and species
...
beha.tro <- site[,c(4, 16, 14)] #pull season, mesohabitat, and trophic guild
...
####Migratory Classes
```{r, trophic guild data, warning=FALSE, include=FALSE}
beha.mig <- site[,c(4, 16, 13)] #pull season, mesohabitat, and trophic guild
...
##Bargraphs of Mesohabitat and Behavioral frequency
...
meso.ssp.summary.2$rn <-  gsub('cactus ', ' ', meso.ssp.summary.3$rn)
meso.ssp.summary.2$rn <-  gsub('shrub ', ' ', meso.ssp.summary.3$rn)
meso.ssp.summary.2$rn <-  gsub('other ', ' ', meso.ssp.summary.3$rn)

meso.ssp.bar.3 <- ggplot(meso.ssp.summary.2, aes(x=mesohabitat, y=mean, fill=rn, position_dodge())) +
  geom_bar(stat = "summary", position = "dodge") +
  geom_errorbar(aes(ymin=0, ymax=mean+sd), position=position_dodge(.9), width = 0) +
  theme_classic() +
  theme(axis.text.x = element_text(size=12)) +
...
beha.bar <- ggplot(beha.ssp.summary, aes(x=behavior, y=mean, fill=rn, position_dodge())) +
  geom_bar(stat = "summary", position = "dodge") +
  geom_errorbar(aes(ymin=0, ymax=mean+sd), position=position_dodge(.9), width = 0) +
  theme_classic() +
  theme(axis.text.x = element_text(size=12)) +
...
ggplot(data = cont) +
  ...
  labs(x = "Behavior Type", y = "Mesohabitat") +
  theme(legend.title = element_blank()) +
...
abun.lm <- lm(abundance~season + (1|walk), data=cov) #lm for season's impact on abundance
summary(abun.lm)
...
rich.glm <- lm(richness~season + (1|walk), data=cov, family="poisson")
summary(rich.glm)
