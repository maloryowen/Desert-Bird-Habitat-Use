...
write_csv(walks.ssp.wide, "~/Masters/Desert-Bird-Habitat-Use/data/species_wide.csv")
...
#for both seasons
#mesos and species
meso.ssp <- site[,c(4, 5, 19, 12)] #pull season, mesohabitat, behavior, and species
...
lon <- -115.66
point <- 1
granites <- data.frame(point, lat, lon)

#california map
cali <- get_map(location = c(lon = -116, lat = 35), zoom = 6, maptype = "terrain", source = 'stamen')
cali <- ggmap(cali) +
scalebar(x.min = -122, x.max = -111,
                   y.min = 30, y.max = 41,
...
cali <- cali +
  geom_point(data = granites, aes(x=lon, y=lat), size = 5) +
...
#map of all bird sightings
deep.sunset <- get_map(location = c(lon = -115.663, lat = 34.7808), zoom = 17, maptype = "satellite", source = "google")
...
site.wide.map <- site.wide.map +
  ...
  labs(title = "Site Wide Species Observations", color = "Order")
...
```{r}
library(sf)
mymap <- st_read("~/Masters/Desert-Bird-Habitat-Use/data/moja_tracts/MOJA_boundary.shp")
mymap_3857 <- st_transform(mymap, 3857) #Transform nc to EPSG 3857 (Pseudo-Mercator, what Google uses)

map <- get_map(location = c(lon = -116, lat = 35), zoom = 8, maptype = 'terrain', source = 'stamen')
...
ggmap(map) + 
  ...
  geom_sf(data = mymap_3857, inherit.aes = FALSE) 
...
spring.matrix <- as.matrix(ssp.spring, row.names=FALSE)
spring.matrix <-dist(scale(t(spring.matrix), center=TRUE, scale=TRUE), method = "manhattan")

#Principle Components analysis of spring species
pca1 <- pcoa(spring.matrix)
summary(pca1)
...
summer.matrix <- as.matrix(ssp.summer, row.names=FALSE)
summer.matrix <-dist(scale(t(summer.matrix), center=TRUE, scale=TRUE), method = "manhattan")

#Principle Components analysis of spring species
pca2 <- pcoa(summer.matrix)
...
# In a biplot of a PCA, species' scores are drawn as arrows 
# that point in the direction of increasing values for that variable
biplot(pca2, choices = c(1,2), type = c("points"), xlim = c(-5,10)) # biplot of axis 1 vs 2
