...
data_div_noBTSP <- as.data.frame(diversity(div_wide_noBTSP[,4:45])) %>% 
  rename(Shannon = "diversity(div_wide_noBTSP[, 4:45])")
div_wide_noBTSP <- div_wide_noBTSP %>% dplyr::select(season, survey, microhabitat)
data_div_noBTSP <- bind_cols(div_wide_noBTSP, data_div_noBTSP)
data_div_noBTSP <- left_join(data_div_noBTSP, weather_survey, by = "survey")
